...
(function($) {
  $(function() {
    ...
    $('.placeholder-1').focusin(function(e) {
      console.log($(this).val());
      if ($(this).val() == 'Ваше имя') {
        $(this).val('');
      }
    });
    $('.placeholder-1').focusout(function(e) {
      if ($(this).val() == '') {
        $(this).val('Ваше имя');
      }
    });
    $('.placeholder-2').focusin(function(e) {
      console.log($(this).val());
      if ($(this).val() == 'Телефон или почта') {
        $(this).val('');
      }
    });
    $('.placeholder-2').focusout(function(e) {
      if ($(this).val() == '') {
        $(this).val('Телефон или почта');
      }
    });
    ...
    $('.border_btn-sub').click(function(event) {
      ...
      if ($textarea.val() == '') {
        ...
        $.ajax({
          ...
          $('body').addClass('body-overflow');
          setTimeout(function() {
            $('.help_block .text textarea').val('').css({
    ...
    $(document).click(function(event) {
      if ($(event.target).closest(".popupOpen, .border_btn").length ||
        $(event.target).closest(
          ".sendForm, .popupError, .popupError2").length)
        return;
    ...
    function initHeaderPosition() {
      ...
      function calcPosition(obj) {
        obj.stop().css({
          left: parseInt(obj.attr('data-top')) - win.scrollLeft()
        });
        win.scroll(function() {
          obj.stop().css({
            left: parseInt(obj.attr('data-top')) - win.scrollLeft()
    ...
    $('.sub-btn').click(function(event) {
      ...
      $("#phone-mail").each(function(event) {
        ...
        if (value != '') {
          ...
          if (!this.value.match(
            ...
            $(this).removeClass('validInput').addClass(
              'errorInput');
            ...
            if ($("#name-input").hasClass('validInput')) {
              setTimeout(function() {
                $('.sendForm form input').removeClass(
                  'validInput');
                $(
                  '.sendForm form #name-input, .sendForm form #phone-mail'
                ).val('');
                $('.sendForm form #name-input').attr(
                  'placeholder', 'Ваше имя');
                $('.sendForm form #phone-mail').attr(
      ...
      if ($("#phone-mail").hasClass('validInput') && $("#name-input")
        .hasClass('validInput')) {
        if (bot.val() !== "") {
          console.log('Go away bot!');
          return false;
